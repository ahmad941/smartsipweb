    ...
    <div class="py-8 text-slate-700" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <!-- Master Classes List Table (Col 8) -->
                <div class="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
                        <div>
                            <h3 class="text-base font-extrabold text-slate-800">Daftar Master Kelas</h3>
                            <p class="text-slate-400 text-xs mt-1">Daftar kelas acuan mandiri yang dapat dipilih oleh semua sekolah dan siswa.</p>
                        </div>
                        
                        <!-- Search Form -->
                        <form method="GET" action="{{ route('admin.classes.index') }}" class="relative sm:w-56">
                            <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama kelas..."
                                class="bg-slate-50 border border-slate-200 rounded-xl text-slate-850 placeholder-slate-400 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white h-10 pl-9 pr-4 w-full focus:outline-none transition-all text-xs" />
                        ...
                        <table class="w-full text-left text-xs">
                            <thead>
                                <tr class="border-b border-slate-100 text-slate-400 uppercase tracking-wider font-extrabold text-[10px]">
                                    ...
                                    <th class="pb-3">Nama Master Kelas</th>
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse ($classes as $index => $cls)
                                    <tr class="hover:bg-slate-50/60 transition-colors">
                                        ...
                                        <td class="py-4 font-extrabold text-slate-800">
                                            <span class="px-3 py-1 bg-indigo-50 border border-indigo-100 text-indigo-700 rounded-lg inline-block">
                                                {{ $cls->name }}
                                            </span>
                                        </td>
                                        <td class="py-4 text-center">
                                            <span class="font-bold text-slate-700">{{ $cls->students_count }} Siswa</span>
                                        </td>
                                        <td class="py-4 text-center">
                                            <div class="flex items-center justify-center gap-1.5">
                                                <button @click="openEdit({{ json_encode($cls) }})" class="px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white border border-indigo-100 hover:border-indigo-600 rounded-lg text-[10px] font-bold transition-all">
                                                    Edit
                                                </button>
                                                <form action="{{ route('admin.classes.destroy', $cls->id) }}" method="POST" onsubmit="return confirm('Hapus Master Kelas {{ $cls->name }}?')">
                                                    @csrf
                                                    @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="4" class="py-12 text-center text-slate-400 font-bold italic">Belum ada data Master Kelas terdaftar.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <!-- Add Master Class Form (Col 4) -->
                <div class="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm h-fit">
                    <h3 class="text-base font-extrabold text-slate-800 mb-4">Tambah Master Kelas Baru</h3>

                    <form action="{{ route('admin.classes.store') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label for="name" class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-2">Nama Kelas *</label>
                            <input type="text" id="name" name="name" required placeholder="Cth: KELAS VII / X-IPA 1"
                                class="bg-slate-50 border border-slate-200 rounded-xl text-slate-850 placeholder-slate-400 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white h-11 px-4 w-full text-xs font-bold transition-all" />
                            @error('name') <p class="text-rose-500 text-2xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <button type="submit" class="w-full mt-2 h-11 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-md active:scale-95 duration-200">
                            Tambah Master Kelas
                        </button>
                    </form>
                </div>
        ...
        <div x-show="editOpen" class="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs" style="display: none;">
            <div @click.outside="editOpen = false" class="bg-white border border-slate-200 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
                <div class="flex justify-between items-center border-b border-slate-100 pb-3">
                    <h4 class="text-base font-extrabold text-slate-800">Edit Master Kelas</h4>
                    <button @click="editOpen = false" class="text-slate-400 hover:text-slate-600">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>

                <form :action="'{{ url('/admin/classes') }}' + '/' + editId" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')

                    <div>
                        <label class="block text-[10px] font-extrabold text-slate-400 uppercase tracking-wider mb-2">Nama Master Kelas *</label>
                        <input type="text" name="name" required x-model="editName"
                            class="bg-slate-50 border border-slate-200 rounded-xl text-slate-850 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white h-11 px-4 w-full text-xs font-bold" />
                    </div>
                    ...
                    <div class="flex justify-end gap-3 border-t border-slate-100 pt-4">
                        ...
                        <button type="submit" class="h-11 px-5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs">Simpan Perubahan</button>
                    </div>
                </form>
            </div>
        </div>
